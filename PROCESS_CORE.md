...
### 9.8 Das Modul «Bewegen» — eine Einstellung, zwei Körnungen

> Vollständige Begründung samt Historie des Scheiterns: **ADR 009**.
> Der Ort selbst — Halter, Beobachtung, Kette — steht in **§15**.

Das Modul bringt Stücke an einen Halter. Es hat genau **EINE** Einstellung: das **Ziel**
(eine Halter-Objektnummer). Kein Transportmodus, keine Quelle, keine Menge, kein
Zeitpunkt — der Modus ist abgeleitet (§15.4), und Quelle, Menge und Zeitpunkt gehören zur
Laufzeit, nicht zur Vorlage. Ein Modul ist eine Vorlage; was es beim Definieren nicht
wissen kann, darf es nicht behaupten.

**Zwei Körnungen, streng getrennt:**

| | |
|---|---|
| **Vorgang** | ein Scan = **eine Instanz** — wie überall im Framework (§4.4). Charge = ein Scan, Einzelserialisierung = n Scans. |
| **Fuhre** | ein Paar (**Ausgangsort → Ziel**). Zwei Ausgangsorte sind **zwei** Fuhren, weil es physisch zwei Transporte sind — mit zwei Preisen, zwei Etiketten, zwei Ankünften. Drei Stücke am selben Ort sind **eine** Fuhre, ein Paket, ein Preis. |

Die Fuhre ist damit **abgeleitet, nicht eingestellt**: sie ist die Gruppierung der
...
**Das Modul wartet auf niemanden.** Jedes Stück läuft in seiner Welle weiter; eine Fuhre,
die noch beim Spediteur liegt, hält die anderen nicht auf. Das ist keine Sonderregel,
sondern §4.4: ein Vorgang ist eine Instanz, und `confirm_step` ist ein **Teil**-Abschluss.

**Der Ablauf am Bildschirm** — je Fuhre eine Zeile:

```
Fuhre 1 · Werk 1 → Werk 3 · 1 Stück · andere Adresse → PAKET vorgeschlagen
Fuhre 2 · Werk 2 → Werk 3 · 1 Stück · andere Adresse → FRACHT vorgeschlagen
```

Liegt alles am selben Ort **und** ist das Ziel dieselbe Adresse, gibt es **keine Fuhre** —
dann steht dort nur, was zu scannen ist. Eine leere Vergabe-Zeile für einen Weg von zwanzig
Metern wäre Papier für nichts.

**Intern** ist damit der kurze Weg: Ort scannen (§15.3), Stücke scannen, fertig.
**Extern** ist derselbe Weg plus die **Vergabe** (§15.5) — ein Klick, ein Scan.

**Ein Ort blockiert nie** (§15.6). Fehlt etwas oder liegt es falsch, ist das kein Zustand
des Auftrags: das Modul ist schlicht nicht fertig, und die Zeile sagt warum.

**Was dieses Modul ausdrücklich NICHT tut** — die drei Altlasten aus ADR 009 §2:

- Es liest den Ort **nie**, um zu entscheiden, **welche** Stücke es anfasst. Was vor dem
  Modul steht, ist was vor dem Modul steht: **null** Fallunterscheidungen.
- Es verteilt keine Mengen auf Orte. «990 im Regal, 10 am Band» sind zehn Einzelinstanzen
  mit anderem Halter — ein `GROUP BY`, keine Map.
- Es legt nichts an. Ein Transport-Auftrag entsteht nur durch den Klick eines Menschen auf
  einen vorausgefüllten Entwurf (§15.7).
...
> Begründung und Historie des Scheiterns: **ADR 009**. Prüfbare Sätze und
> Sackgassen-Analyse: `SYSTEM_LOGIC.md` §7. Das Modul, das den Ort **ändert**, steht in
> §9.8.

Bis hierher beantwortet das Modell «in welchem Auftrag ist ein Stück» und «vor welchem
Modul steht es». Für **freien** Bestand — den Normalzustand eines Lagers — war die Antwort
leer. Das ist die Lücke, die `SYSTEM_INTENT` §5a als grösste zwischen Absicht und Modell
benennt; dieser Abschnitt schliesst sie.

### 15.1 Der Ort ist eine BEOBACHTUNG, kein Schritt

```
unit_places
  id · instance_unit_id · holder_object_id · actor_id · created_at · source
                                                        source ∈ scan | tracking
```

Append-only. Der **aktuelle** Ort ist die letzte Zeile je Stück; die **Historie** fällt
geschenkt an, weil nichts überschrieben wird. Es gibt keine Ortsspalte an der
Einzelinstanz — sie wäre die zweite Wahrheit, und genau daran ist der Vorgänger gescheitert
(ADR 009 §2.2).

**Bewusst NICHT im `process_events`-Log** (§11.3). Eine Ablage muss auch **ohne Auftrag**
möglich sein: der Ereignis-Log hängt an `order_id`/`step_id`, ein Ort ohne Auftrag hätte
dort keine Zeile. Diese Unabhängigkeit **ist** die Robustheit — der Ort funktioniert, wenn
der Prozess stillsteht.

**Lückenlos in dem, was geschah — nicht in der Zeit.** Ein Stück, das drei Jahre im Regal
liegt, hat für diese drei Jahre weiterhin keinen Eintrag; ein Ereignis «es lag weiter da»
wäre Rauschen (`SYSTEM_INTENT` §5b). Neu ist nur, dass die **letzte** Beobachtung eine
Antwort gibt.

### 15.2 Ein Halter ist eine OBJEKTNUMMER — mehr nicht

Kein `location_type` neben der `id`. Objektnummern sind systemweit eindeutig, der Typ ist
daraus ableitbar. Das spart die Validierung, die Typ-Labels und **jede** «unbekannter
Typ»-Fallunterscheidung — im Vorgänger musste ein Alt-Wert `'lagerplatz'` tolerant zu
`None` aufgelöst werden, weil ein entfallener Typ sonst jede Ansicht zerlegt hätte.
...
| Werk Nord · Hauptsitz | ein **Unternehmen** |
| Mitarbeiter · Kunde · DHL · Spediteur | ein **Benutzer** |

Ein Ort wird damit nicht *gepflegt* — er ist ein Datensatz, den es aus einem anderen Grund
ohnehin gibt. DHL legt man an wie jeden anderen Lieferanten.

**Auch ein Mensch ist ein Halter.** Nimmt ein Kollege ein Teil mit und legt es erst morgen
ab, liegt es zwischendurch **bei ihm**. Der alte Ort wäre eine Behauptung über etwas, das
dort nachweislich nicht liegt (G3).

### 15.3 Der Kontext-Scan — der erste Scan ist immer «wo bin ich»

Ohne Vorgabewert, ohne gemerkten Wert. Das kostet **einen Scan mehr je Arbeitsgang** und
ist die einzige Variante, bei der nichts stillschweigend falsch landen kann: jede
Beobachtung nennt eine wirklich gescannte Quelle.

Ein gemerkter Ort («der letzte gilt weiter») wäre die klassische stille Fehlerklasse — wer
den Wechsel vergisst, schreibt den falschen Ort, und niemand bemerkt es, weil nichts
fehlschlägt.

### 15.4 Intern oder extern entscheidet die ADRESSE — EINE Funktion, zwei Nutzer
...
einen Transport. Ihr Zuhause ist `services/address.py`, das die kanonische Adressform für
Person, Unternehmen und Carrier bereits führt.

**Zwei Nutzer, eine Regel:** dieselbe Funktion beantwortet am **Ressourcenmodul** (§9.6)
die Frage «liegt die Komponente hier?». Zwei Ableitungen wären zwei Antworten auf dieselbe
Sache.

**Nichts davon wird gespeichert.** Richtung (outbound/inbound) und Transportklasse sind die
Differenz zweier Halterorte und werden **gerechnet**. Der Vorgänger hat sie gespeichert und
dafür zwei Migrationen gebraucht, um die Werte wieder loszuwerden (ADR 009 §2.4).
...
**Rate-Shopping IST eine Ausschreibung**, sie dauert nur 2 Sekunden statt 2 Tage. Was beim
Paketversand wie ein technischer Sonderweg aussieht, ist derselbe Vorgang wie eine
Frachtanfrage an zwei Spediteure. Die Zustände stehen darum in **einer Registry**, nicht
als `if/else` je Kanal.

**Ab «Vergeben» ist ein Zweiter gebunden** → das System rührt nichts an, es **meldet**. Das
ist keine neue Regel: `EventType.binding` und die Klärungs-Zeile gibt es bereits.
...
fertig, und die Zeile sagt warum (gedeckt · fehlt · liegt woanders). Exakt die Form, die
`StepNeed` heute schon hat — ein Feld, drei Lesarten.

Ebenso gilt die Umkehrung: **der Ort ändert nie einen Status und nie eine Zugehörigkeit.**
Das ist die Robustheitsgarantie, konstruktiv statt geprüft — weil eine Ablage nichts
anfasst ausser dem Ort, muss keine andere Regel im System von ihr wissen. Ein Stück darf
gesperrt, verbaut, verschrottet oder in einer Abweichung sein; sein Ort ist trotzdem
einfach der letzte Scan.

### 15.7 Das System legt NICHTS an — es bietet an

Kein automatisch gesätes Modul, kein automatischer Unter-Auftrag. Genau daran sind die
Begleit-Bewegungen (`companion`/`locked`) und die abgeleitete Bereitstellung gescheitert
(ADR 009 §2.3): ein Transport, den das System selbst anlegt, gehört niemandem — niemand
hat ihn gewollt, niemand erkennt ihn wieder, und wenn er klemmt, fehlt der Mensch, der ihn
auflösen könnte.

Ein Transport entsteht ausschliesslich durch den **Klick eines Menschen** auf einen
vorausgefüllten Entwurf. Am Ressourcenmodul sieht das so aus:

```
4 × Schraube M6 · 200 verfügbar — in Werk 2 · gebraucht in Werk 1
[Andere Instanz wählen]   [Transport]
```

«Transport» öffnet einen vorausgefüllten **Auftragsentwurf** — ein ganz gewöhnlicher
Auftrag, kein Sondertyp. Danach steht in der Zeile: «unterwegs aus Werk 2 · Auftrag
100000920 ▸».

### 15.8 Der Transport-Auftrag ist KEIN Abzweig

Ein Abzweig bedeutet im Bild: Material hat **diese Achse verlassen** und kommt zurück — und
das Bild rechnet damit (jeder `fork` zieht ab, jeder `join` addiert, §8.1a). Ein Transport
bewegt Stücke, die **nie auf der Achse waren**; als Abzweig gezeichnet würde die Bilanz
**falsch rechnen**.
...
### 15.9 Die zwei Fragen

| Frage | Antwort |
|---|---|
| **«Wo ist 100000123?»** | die **Kette** von innen nach aussen: Behälter → Halle → Werk → Anschrift. Sie entsteht, indem der Halter seinerseits nach seinem Halter gefragt wird — zyklensicher und begrenzt. |
| **«Was liegt hier?»** | die **Liste** an jedem Datensatz: alle Stücke, deren letzte Beobachtung auf diese Objektnummer zeigt. |

Beide lesen dieselbe Tabelle aus zwei Richtungen — wie der Ereignis-Log seine zwei
Leserichtungen hat (§11.3). Eine zweite Tabelle für die Gegenrichtung wäre eine zweite
Wahrheit.
