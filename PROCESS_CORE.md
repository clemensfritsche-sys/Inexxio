...
>
> **Legende:** Ein Absatz mit ⚠ ist **noch nicht entschieden** und liegt als Frage in
> §12. Alles ohne ⚠ ist verbindlich.
...
Vorschlag, noch nicht entschieden — siehe **F2** in §12:
...
> ⚠ **Wo dieser Wert wohnt, ist offen (F4).** §10.1 hält Start und Ende bewusst aus der
> Schrittliste heraus; ein konfigurierbarer Wert braucht trotzdem einen Ort.
...
Vorschlag, noch nicht festgeschrieben — siehe **F1** in §12:
...
Status dafür. Eine Farbe ohne Wert ist ehrlicher als ein erfundener Wert.
...
> ⚠ **Damit gibt es keinen gespeicherten Zustand «Entwurf».** Ein Auftrag existiert erst
> ab der Freigabe. Das ist eine Änderung gegenüber der bisherigen Fassung dieses
> Dokuments und gegenüber dem gebauten Stand — siehe **F3** in §12.
...
| # | Schritt |
|---|---|
| 1 | **Freigabebedingungen prüfen.** Nicht erfüllt → Abbruch mit klarer Meldung. |
| 2 | **Datensatz anlegen**, Objektnummer vergeben. |
| 3 | **Exklusivitätsprüfung** (§3). Verletzt → Abbruch, nichts wird angelegt. |
...
> ⚠ **«Keine verbrannte Objektnummer» ist mit einer Postgres-Sequence nicht erreichbar.**
> `nextval` ist bewusst **nicht** transaktional — sonst wäre es kein nebenläufigkeits-
> sicherer Zähler. Ein Rollback nach Schritt 2 lässt eine Lücke im Nummernkreis.
> Siehe **F5** in §12.
...
> ⚠ **Ob diese Regel auch für die Liste der Einzelinstanzen gilt, ist offen (F6).**
...
### 9.2 Technologie-Entscheid: Fluss-Layout + SVG-Overlay auf gemessenen Ankern
...
### 10.1 Ebene 1 — Prozessdefinition (die Modellierung)
...
### 10.2 Ebene 2 — Laufzeit-Zustand (wo steht was)

```
instance_units.status            welchen Status hat das Stück
instance_units.active_order_id   in welchem Auftrag ist es aktiv   (nullable)
...
**`active_order_id` ist zugleich die Durchsetzung der Exklusivität (§3).** Eine Spalte
kann nur einen Wert tragen — «in zwei Aufträgen aktiv» ist damit **nicht darstellbar**,
nicht bloss verboten. Das ist stärker als ein partieller Unique-Index über eine
Zuordnungstabelle, der «aktiv» erst denormalisiert bräuchte, um es sehen zu können.

> ⚠ Damit weiche ich im **Mittel** von der Vorgabe «Constraint / partieller Unique-Index»
> ab, nicht im **Ziel** (Durchsetzung auf Datenbankebene). Siehe **F7** in §12.

Welche Stücke ein **abgeschlossener** Auftrag hatte, steht danach im Ereignis-Log —
`active_order_id` ist dann wieder `NULL`. Das ist genau die Trennung aus §3:
*aktiv* ist exklusiv und steht in der Spalte, *referenziert* ist erlaubt und steht im Log.

### 10.3 Ebene 3 — Ereignis-Log (die eingefrorene Historie)
...
## 11. Bewusst noch nicht definiert
...
**F1 — Die finale Statusliste (aus A1).**
Vorschlag: **genau zwei Werte**, `Freigegeben` und `Im Prozess` (§5.2). `Freigegeben`
**ersetzt** `verfügbar` — ein Zustand, ein Wort. Die übrigen früher vorgeschlagenen
Werte sind zurückgezogen, jeder mit Grund.
*Nebenfrage, die mit entschieden werden muss:* **Womit startet eine frisch angelegte
Einzelinstanz?** Im Code steht heute `new` — ein Platzhalter aus dem Basis-Neuaufbau
ohne Logik. Ist ein neues Stück sofort `Freigegeben`, oder braucht es erst einen
Erzeugungsprozess, um es zu werden? Letzteres hiesse: es gibt einen dritten Wert, und
`new` wäre er.

**F2 — Wann wird ein Stück wieder frei (aus §3)?**
Vorschlag in §3.1: beim Passieren des **Ende-Objekts** (nicht beim Auftragsabschluss),
und beim **Abbruch** über einen eigenen Log-Eintrag.

**F3 — Es gibt keinen gespeicherten Zustand «Entwurf». Bestätigt?**
§6.1 + §6.3 heissen zusammen: ein Auftrag entsteht **erst bei der Freigabe**. Damit
existiert kein Auftrag, der gespeichert, aber nicht freigegeben ist. Das ist eine
**Änderung** gegenüber der bisherigen Fassung dieses Dokuments (§5.1 alt) und gegenüber
dem gebauten Stand: heute gibt es einen «Speichern»-Knopf, der einen Auftrag ohne
Bedingungen anlegt. Der entfällt dann — es gibt nur noch «Freigeben».
Folge, die mitentschieden wird: die **Modellierung des Prozesses passiert im Browser**,
und `process_steps` wird erst bei der Freigabe geschrieben (in derselben Transaktion).

**F4 — Wo wohnt der Endzustand (aus A6)?**
Er soll konfigurierbar modelliert sein, aber §10.1 hält Start und Ende aus der
Schrittliste heraus. Vorschlag: eine Spalte **`orders.end_status`** mit Vorgabewert
`Freigegeben` — dann bleibt die Schrittliste rein und A6 hat seinen einen Ort. Die
Alternative wäre, Start und Ende doch als Zeilen zu führen; das würde §10.1 aufweichen.

**F5 — Objektnummer bei Transaktionsabbruch.**
«Keine verbrannte Objektnummer» ist mit `object_id_seq` nicht erreichbar: `nextval` ist
absichtlich nicht transaktional. Vorschlag: **Reihenfolge in §6.3 tauschen** — die
Exklusivitätsprüfung (3) läuft **vor** der Nummernvergabe (2). Dann verbrennt ein
Verstoss keine Nummer. Bleibt die Datenbank-Absicherung als Netz für den echten
Parallelfall; schlägt sie zu, geht eine Nummer verloren. Alternative: Lücken im
Nummernkreis ausdrücklich als zulässig erklären.

**F6 — Gilt «Definitionen rasten ein» (A5) auch für die Einzelinstanz-Liste?**
A5 spricht von «Löschen und Neuanlegen **des Moduls**». Ich lese es als Regel für
**Modul-Definitionen**. Für die Liste der Einzelinstanzen wäre «löschen und neu anlegen»
dasselbe wie «bearbeiten» — die Regel liefe dort leer. Bestätigst du diese Lesart?

**F7 — Exklusivität als Spalte statt als Unique-Index (§10.2)?**
`instance_units.active_order_id` macht den Verstoss **undarstellbar** statt ihn zu
verbieten. Das erfüllt «Durchsetzung auf Datenbankebene» stärker als ein Index, weicht
aber vom genannten Mittel ab.

**F8 — Artikel-Reiter-Name.**
«Erzeugungsprozess» beibehalten (§8.2)? Begründung dort.
