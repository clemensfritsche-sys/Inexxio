    ...
    <label className="flex flex-col gap-1">
      ...
      {point.type === 'bool' ? (
        ...
        <span className="flex gap-1.5">
          <ThumbChoice active={value === true} tone="var(--success)" disabled={disabled}
            onClick={() => onChange(true)}><ThumbsUp size={15} /></ThumbChoice>
          <ThumbChoice active={value === false} tone="var(--danger)" disabled={disabled}
...
function ThumbChoice({ active, tone, disabled, onClick, children }: {
  active: boolean; tone: string; disabled?: boolean;
