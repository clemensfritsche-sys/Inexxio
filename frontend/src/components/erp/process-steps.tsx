...
import { Plus, Trash2, Link2, User as UserIcon, Info, Eye, Check, ArrowUp, ArrowDown } from 'lucide-react';
import { api } from '@/lib/api';
import type { ArticleProcessStep, ProcessStepMode, StepType, UserProfile } from '@/types';
...
export function ProcessSteps({ articleObjectId, suppliers, readOnly = false }: {
  ...
  const [stepType, setStepType] = useState<StepType>('purchase');
  ...
  function resetForm() {
    setAdding(false); setStepType('purchase'); setMode('supplier'); setSupplierId('');
    setUrl(''); setShared(MANDATORY_FIELD_KEYS); setSamplePercent('100'); setError(null);
  }
  ...
  async function addStep() {
    ...
    try {
      const created = await api.createArticleProcessStep(aid, {
        step_type: stepType,
        mode: stepType === 'purchase' ? mode : undefined,
        supplier_id: stepType === 'purchase' && mode === 'supplier' ? Number(supplierId) : null,
        webshop_url: stepType === 'purchase' && mode === 'webshop' ? url.trim() : null,
        shared_fields: stepType === 'purchase' ? shared : null,
        sample_percent: stepType === 'inspection' ? Math.trunc(Number(samplePercent)) : null,
      });
  ...
  async function move(index: number, dir: -1 | 1) {
    ...
    try {
      await api.updateArticleProcessStep(aid, a.id, { position: b.position });
      await api.updateArticleProcessStep(aid, b.id, { position: a.position });
      const fresh = await api.getArticleProcessSteps(aid);
      setSteps(fresh);
    } catch { /* ignore */ }
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      {readOnly ? (
        ...
        <div style={infoStyle}>
          <Info size={15} style={{ flexShrink: 0, marginTop: 1 }} />
          <span>Diese Schritte definieren den Auftragsprozess (Reihenfolge frei wählbar). Wird ein
            Auftrag mit diesem Artikel freigegeben, startet der Prozess automatisch.</span>
        </div>
      )}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <div style={{ minWidth: 0, flex: 1 }}>
                ...
                <div style={{ marginTop: 3, fontSize: 12, color: '#64748b' }}>
                  ...
                  {s.step_type === 'inspection' && `Stichprobe: ${s.sample_percent ?? 100}% der Menge`}
                </div>
              </div>
              {!readOnly && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 2, flexShrink: 0 }}>
                  <button onClick={() => move(i, -1)} disabled={i === 0} title="Nach oben" style={iconBtn(i === 0)}><ArrowUp size={14} /></button>
                  <button onClick={() => move(i, 1)} disabled={i === steps.length - 1} title="Nach unten" style={iconBtn(i === steps.length - 1)}><ArrowDown size={14} /></button>
                  <button onClick={() => removeStep(s.id)} title="Entfernen" style={{ border: 'none', background: 'none', color: '#94a3b8', cursor: 'pointer', padding: 4 }}><Trash2 size={15} /></button>
                </div>
              )}
            </div>

            {/* Sichtbare Stammdaten – nur Beschaffung */}
            {s.step_type === 'purchase' && (
      ...
      {readOnly ? null : !adding ? (
        ...
        <div style={{ ...cardStyle, flexDirection: 'column', alignItems: 'stretch', gap: 14 }}>
          <Segmented label="Schritt-Typ" value={stepType} onChange={(v) => setStepType(v as StepType)} required
            options={[
              { value: 'purchase', label: 'Beschaffung' },
              { value: 'serialization', label: 'Serialisierung' },
              { value: 'inspection', label: 'Eingangskontrolle' },
            ]} />
          ...
          {stepType === 'serialization' && (
            <div style={infoStyle}>
              <Info size={14} style={{ flexShrink: 0, marginTop: 1 }} />
              <span>Erzeugt nach dem Wareneingang Bestands-Instanzen gemäss Artikel-Einstellung
                (Einzelteil = je Stück eine Nummer, Batch = eine Charge). Keine weitere Konfiguration.</span>
            </div>
          )}

          {stepType === 'inspection' && (
            <TextField label="Prüfumfang (% der Menge)" value={samplePercent} onChange={setSamplePercent}
              required placeholder="z. B. 10" hint="Stichprobe: wie viel Prozent der Menge geprüft werden muss (1–100)" />
          )}
...
function iconBtn(disabled: boolean): React.CSSProperties {
  return {
    border: 'none', background: 'none', color: disabled ? '#cbd5e1' : '#64748b',
    cursor: disabled ? 'default' : 'pointer', padding: 4,
  };
}
