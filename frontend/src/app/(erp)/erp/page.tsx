...
import { Search, Plus, Users, Package, ClipboardList, ChevronDown } from 'lucide-react';
import { cn, userDisplayName } from '@/lib/utils';
import { api } from '@/lib/api';
import type { Article, Order, UserProfile, ErpRecordType } from '@/types';
...
function rowTitle(row: Row): string {
  ...
  if (row.type === 'article') return row.data.name;
  return row.data.title || 'Auftrag';
}
...
function rowSearchText(row: Row): string {
  ...
  return `${row.data.title ?? ''} ${id}`.toLowerCase();
}
...
function FeedItem({ row, sel, onClick }: { row: Row; sel: boolean; onClick: () => void }) {
  ...
  else badge = orderStatusConfig(row.data.status);
...
export default function ErpPage() {
  ...
  useEffect(() => {
    ...
    Promise.allSettled([api.getErpRecords(), api.getArticles(), api.getOrders()])
      .then(([u, a, o]) => {
        ...
        <div className={cn(
          ...
          <div style={{ padding: '14px 14px 10px', borderBottom: '1px solid #F1F5F9' }}>
            ...
            <div style={{ display: 'flex', gap: 5, marginTop: 10, flexWrap: 'wrap' }}>
              {(['user', 'article', 'order'] as ErpRecordType[]).filter((t) => counts[t]).map((t) => {
